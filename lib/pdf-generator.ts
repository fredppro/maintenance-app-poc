...
/**
 * Generates a translated "Folha de Obra" / Maintenance Report PDF using vector operations and Helvetica fonts.
 * Keeps the file size under 100KB by avoiding embedded fonts and heavy image assets.
 */
export async function generateMaintenanceReportPDF(entry: MaintenanceEntry, locale = 'en'): Promise<Buffer> {
  const normalizedLocale = (locale && messagesMap[locale.toLowerCase()]) ? locale.toLowerCase() : 'en';
  ...
  return new Promise((resolve, reject) => {
    try {
      const doc = new PDFDocument({
        size: 'A4',
        margin: 36, // Compact 0.5-inch margins (36 points)
        bufferPages: true, // Enables page numbering later
        info: {
          Title: `${t.title} - ${entry.id}`,
          Author: 'Manusist',
          Subject: t.subject,
        },
      });
      ...
      doc.fillColor('#475569') // slate-600
         ...
         .text('Rua do Progresso, 123, 1000-001 Lisboa', 399, 60, { align: 'right', width: 160 })
         .text('geral@manusist.pt | +351 210 000 000', 399, 70, { align: 'right', width: 160 });
      ...
      doc.fillColor('#64748b')
         .font('Helvetica')
         .fontSize(8.5)
         .text(`${t.recordId}: #${entry.id.substring(0, 8).toUpperCase()}`, 399, 95, { align: 'right', width: 160 })
         .text(`${t.issuedAt}: ${generationTime}`, 399, 107, { align: 'right', width: 160 });

      let currentY = 130;

      // Section Title Helper
      const drawSectionHeader = (title: string, y: number): number => {
        doc.fillColor('#f1f5f9') // slate-100 banner
           .rect(36, y, 523, 18)
           .fill();
      ...
      doc.fillColor('#64748b').font('Helvetica-Bold').fontSize(7.5).text(t.equipment.label, 36, currentY);
      doc.fillColor('#0f172a').font('Helvetica').fontSize(8.5).text(entry.equipment?.name || 'N/A', 36, currentY + 11, { width: 240 });

      doc.fillColor('#64748b').font('Helvetica-Bold').fontSize(7.5).text(t.equipment.category, 36, currentY + 28);
      doc.fillColor('#0f172a').font('Helvetica').fontSize(8.5).text(entry.equipment?.category || 'N/A', 36, currentY + 39, { width: 240 });

      // Right Column (IDs and status)
      doc.fillColor('#64748b').font('Helvetica-Bold').fontSize(7.5).text(t.equipment.code, 300, currentY);
      doc.fillColor('#0f172a').font('Helvetica').fontSize(8.5).text(entry.equipment?.id || 'N/A', 300, currentY + 11, { width: 240 });

      doc.fillColor('#64748b').font('Helvetica-Bold').fontSize(7.5).text(t.equipment.status, 300, currentY + 28);

      // Status Badge translation & design
      const statusText = t.status[entry.status as keyof typeof t.status] || entry.status.toUpperCase();
      ...
      const colors = statusColors[entry.status] || { bg: '#f1f5f9', text: '#475569' };
      ...
      if (entry.assignments && entry.assignments.length > 0) {
        entry.assignments.forEach((assignment, index) => {
          ...
          doc.fillColor('#0f172a')
             .font('Helvetica')
             .fontSize(8)
             .text(assignment.worker.name, 42, currentY + 4, { width: 190, lineBreak: false })
             .text(formatDate(entry.startTime), 240, currentY + 4)
             .text(formatDate(entry.endTime), 360, currentY + 4)
             .font('Helvetica-Bold')
             .text(formatDuration(entry.startTime, entry.endTime), 470, currentY + 4, { align: 'right', width: 80 });

          doc.moveTo(36, currentY + 16)
      ...
      doc.fillColor('#475569')
         ...
         .text(t.materials.quantity, 470, currentY + 3, { align: 'right', width: 80 });
      ...
      if (entry.materials && entry.materials.length > 0) {
        entry.materials.forEach((material, index) => {
          ...
          doc.fillColor('#0f172a')
             ...
             .text(material.reference || '-', 320, currentY + 4, { width: 140, lineBreak: false })
             .font('Helvetica-Bold')
             .text(String(material.quantity), 470, currentY + 4, { align: 'right', width: 80 });
      ...
      doc.fillColor('#64748b').font('Helvetica-Bold').fontSize(7.5).text(t.summary.title, 36, currentY);
      doc.fillColor('#0f172a').font('Helvetica-Bold').fontSize(8.5).text(entry.title, 36, currentY + 11, { width: 240 });

      const typeText = t.types[entry.type as keyof typeof t.types] || entry.type;

      doc.fillColor('#64748b').font('Helvetica-Bold').fontSize(7.5).text(t.summary.type, 300, currentY);
      doc.fillColor('#0f172a').font('Helvetica-Bold').fontSize(8.5).text(typeText, 300, currentY + 11, { width: 240 });

      currentY += 28;

      // Text block for descriptions
      doc.fillColor('#64748b').font('Helvetica-Bold').fontSize(7.5).text(t.summary.notes, 36, currentY);
      ...
      const textHeight = doc.heightOfString(notes, { width: 511, align: 'justify' });
      const boxHeight = Math.max(textHeight + 12, 35);
      
      doc.save();
      doc.rect(36, currentY + 8, 523, boxHeight)
         .lineWidth(0.5)
         .strokeColor('#cbd5e1')
         .stroke();
      doc.restore();
      ...
      doc.fillColor('#475569')
         .font('Helvetica')
         .fontSize(7.5)
         .text(t.signatures.technician, 36, sigY + 5, { width: 174, align: 'center' })
      ...
      for (let i = range.start; i < range.start + range.count; i++) {
        ...
        doc.fillColor('#94a3b8')
           ...
           .text(`${t.footer.page} ${i + 1} ${t.footer.of} ${range.count}`, 489, 792, { align: 'right', width: 70 });
      }
